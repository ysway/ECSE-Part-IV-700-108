...
def a2s():
    ...
    sentencesTuple = namedtuple('Sentences', [
                                'Time', 'Valence', 'Arousal', 'RMS(dB)', 'F0', 'MFCC1', 'MFCC2', 'MFCC3', 'MFCC4', 'MFCC5'])
        ...
        for file in filenames:
            ...
            for row in fileDf.itertuples(index=False):
                if (row[-1] == 'V'):
                    ...
                    currentDf = pd.concat([currentDf, pd.DataFrame(
                        [sentencesTuple(*row[:-1])])], ignore_index=True)
                    ...
                    count += 1
                    # write to dict
                    currentDf.to_csv(outputPath+currentName +
                                     '.csv', index=False)
                    lengthDistributionDf = pd.concat([lengthDistributionDf, pd.DataFrame(
                        {'Name': [currentName], 'Length':[currentDf.shape[0]]})])
                    # empty df
                    currentDf = pd.DataFrame()
                else:
                    pass
    ...
    lengthDistributionDf.to_csv(
        outputPath+'lengthDistribution'+'.csv', index=False)
