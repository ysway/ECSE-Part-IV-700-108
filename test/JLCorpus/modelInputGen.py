...
def main():
    ...
    oldFileName = ''
    stackedDf = pd.DataFrame(columns=list(
        ['Time', 'Valence', 'Arousal', 'RMS(dB)', 'F0', 'MFCC1', 'MFCC2', 'MFCC3', 'MFCC4', 'MFCC5', 'FileName']))
    currentFeatureDf = pd.DataFrame(columns=list(
        ['Time', 'RMS(dB)', 'F0', 'MFCC1', 'MFCC2', 'MFCC3', 'MFCC4', 'MFCC5', 'FileName']))
    for row in inputDf.itertuples(index=False):
        ...
        if oldFileName != currentFileName:
            ...
            try:
                ...
                stackedDf = stackedDf.append(
                    currentFeatureDf, ignore_index=True)
                # SAVE END #
            except:
                pass
            ...
            nextVaDfValue = pd.DataFrame(
                {'Valence': [row[2]], 'Arousal': [row[3]]})
