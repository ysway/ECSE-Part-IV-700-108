...
def sessionProcess(dir, filenames, outputPath):
    ...
    if len(filenames) == 4:
        TU_Features, TU_VA = alignDF(pd.read_csv(
            dir+'TU_Features.csv'), pd.read_csv(dir+'TU_VA.csv'))
        TU_VA.join(TU_Features.drop(['Time'], axis=1)).to_csv(
            outputPath+'sema'+currentSession+'tu.csv', index=False)

        TO_Features, TO_VA = alignDF(pd.read_csv(
            dir+'TO_Features.csv'), pd.read_csv(dir+'TO_VA.csv'))
        TO_VA.join(TO_Features.drop(['Time'], axis=1)).to_csv(
            outputPath+'sema'+currentSession+'to.csv', index=False)
    else:
        nU = 0
        nO = 0
